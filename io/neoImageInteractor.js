...
// provides
goog.provide('X.neoImageInteractor');
...
X.neoImageInteractor = function(element, isThumbnail) {
   ...
   * @const
   */
  this._classname = 'neoImageInteractor';
  ...
  this._rollHandler = null;
  
  this._rollAmount = 0;
};
// inherits from interactor
goog.inherits(X.neoImageInteractor, X.interactor3D);


/**
 * @inheritDoc
 */
X.neoImageInteractor.prototype.init = function() {
 ...
 * @inheritDoc
 */
X.neoImageInteractor.prototype.onMouseWheel_ = function(event) {

  //goog.base(this, 'onMouseWheel_', event);
  
  if (!goog.isDefAndNotNull(this._sliceActive)) return;
  ...
  this._rollAmount += (event.deltaY>0)?1:-1;
  
  function sendResliceEvent(interactor) {
    var e = new X.neoEvent.ResliceEvent();
    e._dir = interactor._sliceActive;
    
    // we empty the buffer
    e._z = interactor._rollAmount;
    interactor._rollAmount = 0;
    interactor.dispatchEvent(e);
  }
  
  var interactor = this;
  if (Math.abs(this._rollAmount)<3) {
    clearTimeout(this._rollHandler);
    this._rollHandler = setTimeout(function(){
      sendResliceEvent(interactor);
    }, 100);
  } else {
    sendResliceEvent(interactor);
  }
  ...
  // set the params
  e._dir = this._sliceActive;
  e._z = (event.deltaY>0)?1:-1;
 ...
 * @inheritDoc
 */
X.neoImageInteractor.prototype.onMouseDown_ = function(event) {
 ...
 * @private
 */
X.neoImageInteractor.prototype.onMouseUp_ = function(event) {
  if (event.button == goog.events.BrowserEvent.MouseButton.LEFT) {
 ...
 * @inheritDoc
 */
X.neoImageInteractor.prototype.onMouseMovementInside_ = function(event) {
  ...
  if (this._isThumbnail) {
    ...
    if (this._leftButtonDown && shiftDown && !ctrlDown && this._rotationActive) {
      ...
      if (goog.isDefAndNotNull(this._sliceActive)) {
        // slices moving
        var e = new X.neoEvent.ResliceEvent();
        e._dir = this._sliceActive;
        e._x = distance.x * 100;
 ...
 * @public
 */
X.neoImageInteractor.prototype.onDblClick = function(event) {
};
 ...
 */
X.neoImageInteractor.prototype.onDblClick_ = function(event) {
 ...
 * @inheritDoc
 */
X.neoImageInteractor.prototype.onKey_ = function(event) {
 ...
 * @private
 */
X.neoImageInteractor.prototype.onKey_Up = function(event) {
...
goog.exportSymbol('X.neoImageInteractor', X.neoImageInteractor);
goog.exportSymbol('X.neoImageInteractor.prototype.init', X.neoImageInteractor.prototype.init);
goog.exportSymbol('X.neoImageInteractor.prototype.onDblClick', X.neoImageInteractor.prototype.onDblClick);
